...
// トークンは URL から拾って全部の要求に付け直す。Cookie にも入るが、
// それを当てにすると Cookie を消した端末で画面が黙って壊れる。
...
async function boot() {
  S.state = await (await fetch(u("/api/state"))).json();
  const c = await (await fetch(u("/api/corrections"))).json();
...
function loadStill(n) {
  ...
  img.src = u("/frame", Object.assign({ n }, raw));
}
