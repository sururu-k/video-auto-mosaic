...
3. **出力側の独立検証を出荷ゲートにする。** 学習に使っていない別モデルで再スキャンし、何も検出されないことを出荷条件にする
   （`tools/verify_output.py` として実装したが、現状の構成では機能しない。既定が本番と同じ重みなので、
   検出器自身が盲目な場所は再検出でも同じく盲目になる。詳細は issue #13 / `docs/05-overnight-log.md`）
...
実装したが現状の構成では機能しない:
- 出力側の独立再スキャンによる出荷ゲート（`tools/verify_output.py`。既定モデルが本番と
  同じ NudeNet 系の重みで、検出器の盲点を検出器自身では検証できない。issue #13）
