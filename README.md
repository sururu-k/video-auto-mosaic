...
- したがって AGPL-3.0 / GPL-3.0 のライブラリ・重みを**そのまま使える**（開示義務は配布時にのみ発生する）。Ultralytics Enterprise ライセンスの購入は不要
- 唯一の例外は NSFW-API/NSFW_Segmentation で、**ライセンス未指定＝許諾そのものが存在しない**。評価・PoC 止まりにする
